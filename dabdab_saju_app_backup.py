...
def _saju_table_html(pillars, d, analyzer):
    order = ['시주', '일주', '월주', '년주']
    pillar_map = {p[0]: p for p in pillars}
    
    html = "<div style='overflow-x:auto; margin-bottom: 20px;'><table style='width:100%; border-collapse: collapse; text-align:center; font-size:13px;'>"
    html += "<tr style='background-color:#f5f4f1; font-weight:bold;'>"
    html += "<td style='padding:10px; border:1px solid #e0e0e0; width:20%;'>구분</td>"
    for label in order:
        html += f"<td style='padding:10px; border:1px solid #e0e0e0; width:20%;'>{label}</td>"
    html += "</tr>"
    
    def td(content, bg="transparent", color="#333", extra_style=""):
        return f"<td style='padding:8px; border:1px solid #e0e0e0; background-color:{bg}; color:{color}; {extra_style}'>{content}</td>"
    
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>십성(천간)</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[1] == '-': html += td("-")
        else:
            deity = "본인(일간)" if p[3] is None else determine_ten_deity(analyzer.day_master, p[1], False)
            html += td(deity)
    html += "</tr>"
    
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>음양오행</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[1] == '-': html += td("-")
        else:
    ...
    html += "<tr><td style='background-color:#f9f9f9; font-weight:bold; border:1px solid #e0e0e0;'>천간</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[1] == '-': html += td("-", extra_style="font-size:24px; font-weight:900;")
        else:
            bg, fg = ELEMENT_COLORS[STEM_INFO[p[1]]['element']]
            k_stem = STEM_INFO[p[1]]['name'][0]
            html += td(f"{k_stem}{p[1]}", bg=bg, color=fg, extra_style="font-size:24px; font-weight:900; letter-spacing:2px; padding:15px; border-radius:8px;")
    html += "</tr>"
    
    html += "<tr><td style='background-color:#f9f9f9; font-weight:bold; border:1px solid #e0e0e0;'>지지</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[2] == '-': html += td("-", extra_style="font-size:24px; font-weight:900;")
        else:
            bg, fg = ELEMENT_COLORS[BRANCH_INFO[p[2]]['element']]
            k_branch = BRANCH_INFO[p[2]]['k_name'][0]
            html += td(f"{k_branch}{p[2]}", bg=bg, color=fg, extra_style="font-size:24px; font-weight:900; letter-spacing:2px; padding:15px; border-radius:8px;")
    html += "</tr>"
    ...
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>십성(지지)</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[2] == '-': html += td("-")
        else:
            deity = determine_ten_deity(analyzer.day_master, p[2], True)
            html += td(deity)
    html += "</tr>"
    
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>십이운성</td>"
    pos_idx = {k: i for i, k in enumerate(d['positions'])}
    for label in order:
        p = pillar_map.get(label)
        if not p or p[2] == '-': html += td("-")
        else:
            i = pos_idx[p[4]]
            uns = d['unseong_list'][i]
            html += td(uns)
    html += "</tr>"
    
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>십이신살</td>"
    for label in order:
        p = pillar_map.get(label)
        if not p or p[2] == '-': html += td("-")
        else:
            i = pos_idx[p[4]]
            sinsal = d['sinsal_list'][i]
            html += td(sinsal)
    html += "</tr>"
    
    html += "<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>귀인</td>"
    sinsal_data = d['sinsal']
    for label in order:
        p = pillar_map.get(label)
        if not p or p[2] == '-': 
            html += td("-")
    ...
    gm = d['gongmang_info']
    gm_str = '·'.join(gm['branches']) if gm['branches'] else '-'
    html += f"<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>기타정보</td><td colspan='4' style='padding:8px; border:1px solid #e0e0e0; text-align:left; padding-left:15px; font-size:12px;'>"
    html += f"<b>납음오행:</b> {d['naeum_info']['name']} &nbsp;|&nbsp; <b>격국:</b> {d['guckguk_info']['name']} &nbsp;|&nbsp; <b>공망:</b> {gm_str} &nbsp;|&nbsp; <b>월령:</b> {d['deukryeong_info']['status']}"
    html += "</td></tr>"

    strength = d['adj_strength']
    season = strength['season_info']
    season_line = " ".join(f"{e}({season['status_by_element'][e]})" for e in ['木', '火', '土', '金', '水'])
    html += f"<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>신강신약</td><td colspan='4' style='padding:8px; border:1px solid #e0e0e0; text-align:left; padding-left:15px;'>"
    html += f"<b>{strength['strength']}</b> <span style='font-size:11px; color:#666;'>(월령 가중 반영 비율 {strength['helper_ratio']:.1f}% / 아군 기운: {'·'.join(strength['helper_elements'])})</span><br>"
    html += f"<span style='font-size:11px; color:#888;'>월령 왕상휴수사: {season_line}</span>"
    html += "</td></tr>"
    
    html += f"<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>용신(用神)</td><td colspan='4' style='padding:8px; border:1px solid #e0e0e0; text-align:left; padding-left:15px; font-size:12px;'>"
    html += f"<b>억부용신:</b> {d['eokbu_elem']}"
    if d['johu_info']['needed']:
        urgent_tag = " (시급)" if d['johu_info'].get('urgent') else ""
        html += f" &nbsp;|&nbsp; <b>조후용신:</b> {d['johu_info']['element']}{urgent_tag}"
    if d['tonggwan_info']['needed']:
        html += f" &nbsp;|&nbsp; <b>통관용신:</b> {d['tonggwan_info']['element']}"
    html += "</td></tr>"
    ...
    sinsal_str = ", ".join(f"{b}" for b in badges) if badges else "해당하는 주요 신살이 없습니다."
    
    html += f"<tr><td style='background-color:#f5f4f1; font-weight:bold; border:1px solid #e0e0e0;'>주요 신살</td><td colspan='4' style='padding:8px; border:1px solid #e0e0e0; text-align:left; padding-left:15px; font-size:12px;'>"
    html += f"<span>{sinsal_str}</span>"
    html += "</td></tr>"
...
def render_result_screen():
    ...
    st.subheader("사주 원국 및 종합 분석표")
    table_html = _saju_table_html(pillars, d, analyzer)
    st.markdown(table_html, unsafe_allow_html=True)

    st.write("---")
    st.markdown("<h4 style='color:#333; margin-bottom:15px; font-size:18px;'>📊 실질 오행(五行) 에너지 분포도 (합화 반영)</h4>", unsafe_allow_html=True)
    
    adj = d['adj_scores']
    total_ohaeng = sum(adj.values()) or 1
    
    bar_html = "<div style='font-size:14px; background:#fff; padding:15px; border-radius:8px; margin-bottom:10px; border:1px solid #eee;'>"
    for elem in ['木', '火', '土', '金', '水']:
        pct = adj[elem] / total_ohaeng * 100
        bg, _ = ELEMENT_COLORS[elem]
        bar_html += f"<div style='display:flex; align-items:center; margin-bottom:8px;'>"
        bar_html += f"<div style='width:60px; font-weight:bold;'>{elem}</div>"
        bar_html += f"<div style='flex:1; background:#e9ecef; border-radius:10px; overflow:hidden; margin:0 15px;'>"
        bar_html += f"<div style='height:20px; width:{pct}%; background-color:{bg};'></div></div>"
        bar_html += f"<div style='width:50px; text-align:right; font-weight:bold;'>{pct:.1f}%</div>"
        bar_html += "</div>"
    bar_html += "</div>"
